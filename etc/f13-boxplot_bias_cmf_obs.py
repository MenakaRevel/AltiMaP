...
for point in np.arange(pnum):
    ...
    if disttomouth[point] <= 1.0:
        data0.append(BIAS)
        print ("<=1km",pname[point], BIAS, disttomouth[point])
    elif disttomouth[point] <= 5.0:
        data1.append(BIAS)
        print ("<=5km",pname[point], BIAS, disttomouth[point])
    else:
        data2.append(BIAS) 
        print (">5km",pname[point], BIAS, disttomouth[point])
#----
...
ax.set_ylabel('Long-term Bias $(m)$', color='k',fontsize=8)
