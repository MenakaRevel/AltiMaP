...
# get CMF results
# wse_cmf, wse_cmf_max, wse_cmf_min, wse_cmf_max_loc, wse_cmf_min_loc = read_wse_multi(coll_xx, coll_yy, syear, eyear, coll_number, coll_lat, coll_lon)

# # get upstream of xlist and ylist
# uwse_cmf, uwse_cmf_max, uwse_cmf_min, uwse_cmf_max_loc, uwse_cmf_min_loc = read_wse_multi(uxlist, uylist, syear, eyear, numbers, llats, llons)

############
# observations
############
coll_wse[:, :, 0] = coll_wse0
...
for i in range(len(coll_number)):
    ...
    # CMF data
    ############
    coll_wse[:,count,1] = coll_cmf0[:,i] - leled[i]
    # coll_wse_max[:,count,1] = wse_cmf_max.T[:,i]
    # coll_wse_min[:,count,1] = wse_cmf_min.T[:,i] 
    
    print (coll_name[i][0], rivseq[coll_yy[i],coll_xx[i]], leled[i], np.mean(coll_cmf0[:,i]), np.mean(coll_cmf0[:,i])-leled[i])       
    count += 1
...
if TAG=="HydroWeb":
    fname='hydroweb_cmf_elediff_daily_wse_VIC_BC.nc'
if TAG=="CGLS":
    fname='cgls_cmf_elediff_daily_wse_VIC_BC.nc'
if TAG=="ICESat":
    fname='icesat_cmf_elediff_daily_wse_VIC_BC.nc'
if TAG=="HydroSat":
    fname='hydrosat_cmf_elediff_daily_wse_VIC_BC.nc'
#-------
...
# str_out   = stringtochar(np.array(list(coll_name)))#, dtype='object')
stname[:] = coll_name #str_out
# for i in range(count) :
#     nana=coll_name[i][0]
#     print (nana, np.asarray(list(nana.ljust(strlen))))
...
ctry[:] = coll_country #str_out
# for i in range(count) :
#     nana=coll_country[i][0]
...
# str_out = stringtochar(np.array(coll_duration)) #, dtype='object'))
sat [:] = coll_satellite #str_out
# for i in range(count) :
#     nana=coll_satellite[i][0]
...
# str_out = stringtochar(np.array(coll_duration)) #, dtype='object'))
stat [:] = coll_status #str_out
# for i in range(count) :
#     nana=coll_status[i][0]
...
v=o.createVariable('sfcelv_cmf', 'f', ('nbdays', 'stations'))
v.longname='water surface elevation at the CMF estimation'
for i in range(count):
    v[:,i] = coll_wse[:,i,1]
