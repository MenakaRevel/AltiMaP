...
# import src.read_hydroweb as hweb
# import src.read_cgls as cgls
# import src.read_hydrosat as hsat
# import src.read_icesat as isat
# import src.read_grrats as grt
#=============================
def mkdir(path):
    try:
        os.makedirs(path)
...
with open(fname, "r") as f:
    ...
    for line in lines[1::]:
        ...
        #---------------------------
        lnames.append(name)
        lupare.append(uparea[iy,ix])
#=============================
lnames=np.array(lnames)
lupare=np.array(lupare)
N=float(len(lnames))
thresholds = [1e13,1e12,1e11,1e10,1e9,1e8]
lupamt=[]
for threshold in thresholds:
    upa_amt=(np.sum((lupare <= threshold)*1.0)/N)*100.0
    print (upa_amt)
    lupamt.append(upa_amt)
#=============================
mkdir("./fig")
mkdir("./fig/criteria")
#=============================
...
#-----------------------------
lupamt=np.array(lupamt)
ax.plot(thresholds,lupamt,marker="*",color="blue",linewidth=1.0,alpha=1)
ax.set_xscale('log')
# scentific notaion
# ax.ticklabel_format(style="sci",axis="x",scilimits=(0,0))
# ax.xaxis.major.formatter._useMathText=True 
# ax.set_xticklabels(thresholds,fontsize=8)
ax.set_ylabel(r'% of data', color='k',fontsize=8)
ax.set_xlabel('Uparea Threshold', color='k',fontsize=8)
ax.tick_params('y',labelsize=6, colors='k')
ax.tick_params('x',labelsize=6, colors='k')
plt.savefig("./fig/criteria/upareas.png",dpi=500)