...
with open(fname, "r") as f:
    ...
    for line in lines[1::]:
        ...
        ix   = ix0 - 1
        iy   = iy0 - 1
        ux, uy = upstream(ix0,iy0,nextX.T,nextY.T,uparea.T)
        if ux==-9999 or uy==-9999:
            ele1=0.0
            ele0=0.0
        else:
            ele1=elevtn[uy-1,ux-1] 
            ele0=elevtn[iy,ix]
        #---------------------------
        lnames.append(name)
        leledf.append(ele1-ele0)
#=============================
lnames=np.array(lnames)
leledf=np.array(leledf)
N=float(len(lnames))
thresholds = [2.0,5.0,10.0,20.0,30.0,40.0]
lelemt=[]
for threshold in thresholds:
    ele_amt=(np.sum((leledf <= threshold)*1.0)/N)*100.0
    print (ele_amt)
    lelemt.append(ele_amt)
#=============================
mkdir("./fig")
mkdir("./fig/criteria")
#=============================
...
#-----------------------------
lupamt=np.array(lelemt)
ax.plot(thresholds,lelemt,marker="2",color="blue",markerfacecolor="red",markeredgecolor="red",linestyle="--",linewidth=1.0,alpha=1)
# ax.set_xscale('log')
# scentific notaion
...
ax.set_xlabel('Slope Threshold', color='k',fontsize=8)
ax.tick_params('y',labelsize=6, colors='k')
ax.tick_params('x',labelsize=6, colors='k')
plt.savefig("./fig/criteria/slpoes.png",dpi=500)