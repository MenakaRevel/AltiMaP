...
for point in np.arange(pnum):
    ...
    ## slope > 20 m --> dist_to_mouth <  1 km
    ## slope > 10 m --> dist_to_mouth <  5 km
    ## slope >  5 m --> dist_to_mouth <  10 km
    ## slope <  5 m --> dist_to_mouth <  All
    # print (cmf_mean,obs_mean,BIAS)
    if slp > 20.0 and disttomouth[point] <= 1.0:
        data0.append(RMSE1)
        # print ("slope: ",slp,"<=1km", disttomouth[point],pname[point], RMSE1)
    elif slp > 10.0 and disttomouth[point] <= 5.0:
        data1.append(RMSE1)
        # print ("slope: ",slp,"<=5km", disttomouth[point],pname[point], RMSE1)
    elif slp > 5.0 and disttomouth[point] <= 10.0:
        data2.append(RMSE1)
        # print ("slope: ",slp,"<=5km", disttomouth[point],pname[point], RMSE1)
    elif slp <= 5.0 and disttomouth[point] < 1e20:
        data3.append(RMSE1)
        # print ("slope: ",slp,"<=5km", disttomouth[point],pname[point], RMSE1)
    else: 
        print ("No belong to criteria --->","slope: ",slp,"<=5km", disttomouth[point],pname[point][0], RMSE1)
#----
colors=['xkcd:pastel blue','xkcd:aqua green','xkcd:soft pink','xkcd:tan']
labels=["slope$>20m$\ndistance$<1km$","slope$>10m$\ndistance$<5km$","slope$>5m$\ndistance$>10km$","slope$<5m$\ndistance$:All$"]
# plt.clf()  
# plt.close() 
# figure in A4 size
